...
// Auxiliary Function process_octree (parallelized version)
void process_octree(float (*fn)(float, float, float),
  ...
  // PARALLEL SECTION: Only parallelize at higher levels to avoid overhead
  if (depth >= 3) {
    // Create local storage for each of the 8 octants
    vector<vector<Vector>> local_vertices(8);
    vector<vector<vector<int>>> local_faces(8);

    #pragma omp parallel for
    for (int i = 0; i < 8; i++) {
      float lx0, ly0, lz0, lx1, ly1, lz1;
      
      // Determine bounds for each octant
      switch(i) {
        case 0: lx0=x0; ly0=y0; lz0=z0; lx1=mx; ly1=my; lz1=mz; break;
        case 1: lx0=mx; ly0=y0; lz0=z0; lx1=x1; ly1=my; lz1=mz; break;
        case 2: lx0=x0; ly0=my; lz0=z0; lx1=mx; ly1=y1; lz1=mz; break;
        case 3: lx0=mx; ly0=my; lz0=z0; lx1=x1; ly1=y1; lz1=mz; break;
        case 4: lx0=x0; ly0=y0; lz0=mz; lx1=mx; ly1=my; lz1=z1; break;
        case 5: lx0=mx; ly0=y0; lz0=mz; lx1=x1; ly1=my; lz1=z1; break;
        case 6: lx0=x0; ly0=my; lz0=mz; lx1=mx; ly1=y1; lz1=z1; break;
        case 7: lx0=mx; ly0=my; lz0=mz; lx1=x1; ly1=y1; lz1=z1; break;
      }
      
      process_octree(fn, lx0, ly0, lz0, lx1, ly1, lz1,
                    depth-1, isoValue, local_vertices[i], local_faces[i], dx, dy, dz);
    }

    // Merge results from all octants
    for (int i = 0; i < 8; i++) {
      ...
      for (const auto& face : local_faces[i]) {
        faces.push_back({face[0] + offset, face[1] + offset, face[2] + offset});
      }
    }
  } else {
    // Sequential execution for lower depths
...
// More complex implicit function (multiple shapes)
float complexFunction(float x, float y, float z) {
  // Sphere
  float r1 = 3.0f;
  float sphere = x*x + y*y + z*z - r1*r1;
  
  // Torus
  float R = 5.0f;
  float r2 = 1.5f;
  float t = sqrt(x*x + z*z) - R;
  float torus = t*t + y*y - r2*r2;
  
  // Combine with smooth min
  float k = 0.5f;
  float h = max(k - abs(sphere - torus), 0.0f) / k;
  return min(sphere, torus) - h*h*h*k*(1.0f/6.0f);
}
...
int main(int argc, char* argv[]) {
  ...
  // Read resolution from command line if provided
  if (argc > 1) {
    resolution = atoi(argv[1]);
  }
  
  // Read number of threads from command line if provided
  if (argc > 2) {
