...
# Crear carpeta images_hard si no existe
if not os.path.exists("images_hard"):
    os.makedirs("images_hard")

# Cargar datos
df_strong = pd.read_csv("strong.csv")
df_weak   = pd.read_csv("weak.csv")

# Filtrar solo modo HARD
df_strong = df_strong[df_strong["mode"] == "hard"]
df_weak   = df_weak[df_weak["mode"] == "hard"]
...
def plot_method(method):
    ...
    fig.suptitle(f"{method} - Modo HARD (Memoria Compartida)", fontsize=16, fontweight="bold")
    ...
    filename = f"v2/{method}_hard_scaling.png"
